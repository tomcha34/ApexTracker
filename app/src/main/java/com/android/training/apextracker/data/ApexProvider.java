...
/**
 * {@link ContentProvider} for Apex Tracker app.
 */
public class ApexProvider extends ContentProvider {

    /** Tag for the log messages */
    public static final String LOG_TAG = ApexProvider.class.getSimpleName();
    ...
    static {
        ...
        // The content URI of the form "content://com.android.training.apextracker/games" will map to the
        // integer code {@link #GAMES}. This URI is used to provide access to MULTIPLE rows
        // of the games table.
        sUriMatcher.addURI(ApexContract.CONTENT_AUTHORITY, ApexContract.PATH_GAMES, GAMES);
        ...
        // For example, "content://com.android.training.apextracker/games/#3" matches, but
        // "content://com.android.training.apextracker/games" (without a number at the end) doesn't match.
        sUriMatcher.addURI(ApexContract.CONTENT_AUTHORITY, ApexContract.PATH_GAMES + "/#", GAME_ID);
    }

    /** Database helper object */
    private ApexDbHelper mDbHelper;

    @Override
    public boolean onCreate() {
        mDbHelper = new ApexDbHelper(getContext());
        return true;
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case GAME_ID:
                ...
                // For an example URI such as "content://com.android.training.apextracker/games/#3",
    ...
    private Uri insertGame(Uri uri, ContentValues values) {
        ...
        // If the kills is provided, check that it's greater than or equal to 0
        Integer kills = values.getAsInteger(GameEntry.COLUMN_GAME_KILLS);
        if (kills != null && kills < 0) {
            throw new IllegalArgumentException("Game requires valid kills");
        }

        // No need to check the place, any value is valid (including null).
    ...
    private int updateGame(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link GameEntry#COLUMN_GAME_KILLS} key is present,
        // check that the kill value is valid.
        if (values.containsKey(GameEntry.COLUMN_GAME_KILLS)) {
            // Check that the kill is greater than or equal to 0.
            Integer kills = values.getAsInteger(GameEntry.COLUMN_GAME_KILLS);
            if (kills != null && kills < 0) {
                throw new IllegalArgumentException("Game requires valid kills");
            }
